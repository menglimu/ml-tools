...
class GenerateApis {
    url = "http://10.10.77.129:8080"; // 接口的地址
    API_PATH = path.resolve(__dirname, "./modules_generate"); // 接口保存的路径
    // 当前处理的group
    group;
    // 获取所有接口组
    async getAll() {
    ...
    async getGroup(url, name) {
        try {
            ...
            // 一个module为一个文件
            let modules = tags.map(tag => ({ ...tag, interfaces: [] }));
            // 找不到tag的接口放这里面
            modules.push({
                name: "other",
            ...
            urls.forEach(url => {
                ...
                let interfaces = methods.map(method => ({
                    ...paths[url][method],
